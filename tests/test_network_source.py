    ...
    gpd.GeoDataFrame(
        {
            ...
            "geometry": [
                LineString(
                    [(57.5, -20.2), (57.55, -20.15), (57.6, -20.2)]
                )
            ],
        },
        crs="EPSG:4326",
...
def test_build_inferred_network_for_region_uses_osm_fixtures(tmp_path, monkeypatch):
    ...
    assert outputs.spatial_nodes.parent == (
        output_dir / "inferred-osm-rodrigues" / "geoparquet"
    )
    ...
    assert any(
        "Model line length differs" in warning
        for warning in inferred_validation["warnings"]
    )
    length_check = inferred_validation["checks"][
...
def test_build_inferred_uses_osm_substations_and_generators_as_roots(
...
def test_build_inferred_provided_uses_only_provided_power_assets(
    tmp_path, monkeypatch
):
    input_dir = tmp_path / "processed" / "energy" / "provided"
    _write_base_inputs(input_dir)
