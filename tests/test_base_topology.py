...
from mu_star_energy.base_topology import CEB_SUBSTATION_NAMES, derive_base_topology
...
def test_base_topology_uses_station_as_one_three_way_junction():
    ...
    center_lines = result.lines[
        result.lines["bus0"].eq("CENTER") | result.lines["bus1"].eq("CENTER")
    ]
...
def test_ceb_base_regression_restores_reviewed_loops_and_amaury_junction():
    ...
    substation_path = (
        root / "data/1-processed/energy/provided/snapped_substations.parquet"
    )
    ...
    graph = nx.MultiGraph(
        result.lines[["bus0", "bus1"]].itertuples(index=False, name=None)
    )
    retained_parts = result.lines.groupby("source_route_part_id", dropna=True)[
        "length_km"
    ].sum()
