...
def test_operational_network_rejects_missing_generator_marginal_cost():
    ...
    lines = geopandas.GeoDataFrame(
        {
            ...
            "length_km": [10.0],
            "s_nom_mva": [100.0],
            "geometry": [shapely_geometry.LineString([(57.5, -20.2), (57.6, -20.2)])],
        },
        crs="EPSG:4326",
    )
    ...
    service_weights = pd.DataFrame({"bus_id": ["A", "B"], "service_weight": [0.5, 0.5]})

    with pytest.raises(ValueError, match="running costs are incomplete"):
        build_operational_network(buses, lines, generators, demand, service_weights)
...
def test_operational_network_uses_profile_values_and_time_step_duration():
    ...
    network = build_operational_network(buses, lines, generators, demand, service_weights)
...
def test_operational_network_rejects_irregular_profile_times():
    demand = pd.DataFrame(
        ...
        index=pd.DatetimeIndex(["2025-01-01 00:00", "2025-01-01 01:00", "2025-01-01 03:00"]),
    )
