...
def _polygon_features(path: Path, label: str) -> gpd.GeoDataFrame:
    ...
    if unsupported:
        raise ValueError(
            f"{label} contains unsupported geometry types: {sorted(unsupported)}"
        )
    return frame
...
def _target_points(
    ...
    return gpd.GeoDataFrame(
        {
            "target_id": [
                f"nightlight_{number:06d}" for number in range(1, len(rows) + 1)
            ],
...
def _read_nightlights(
    ...
    with rasterio.open(path) as source:
        if source.count != 1 or source.crs is None:
            raise ValueError(
                "Nightlights raster must have one band and a coordinate system"
            )
...
def build_nightlight_targets(
    ...
    *,
    ...
    radiance, filter_valid, analysis_valid, transform, raster_crs = _read_nightlights(
        nightlights_path, aoi
    )
    ...
    target_points = _target_points(
        nightlight_mask, radiance, filtered, transform, raster_crs
    )
