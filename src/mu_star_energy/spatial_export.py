...
def _prepare_geometry(
    ...
    *,
    ...
    if not geometry.geom_type.eq(geometry_type).all():
        ...
        raise ValueError(
            f"{label} geometries must all be {geometry_type}; found {found}"
        )
    ...
    if invalid.any():
        ...
        if label == "lines" and "source" in frame:
            logical_anchor = frame["source"].astype("string").str.endswith(
                "_anchor", na=False
            )
            logical_anchor &= geometry.map(
                lambda value: len(set(value.coords)) == 1
            )
...
def _prepare_nodes(
    ...
    *,
    ...
    nodes = gpd.GeoDataFrame(
        {
            ...
            "carrier": _column(buses, "carrier", default="AC", dtype="string").fillna(
                "AC"
            ),
            "v_nom_kv": (
                model_voltage
                if publish_voltage
                else pd.Series(pd.NA, index=buses.index, dtype="Float64")
            ),
            ...
            "anchor_distance_m": pd.to_numeric(
                buses["anchor_distance_m"]
                if "anchor_distance_m" in buses
                else pd.Series(pd.NA, index=buses.index),
                errors="coerce",
            ).astype("Float64"),
        },
        geometry=geometry,
    ...
    return nodes.loc[:, NODE_COLUMNS].sort_values(
        "asset_id", kind="mergesort"
...
def _prepare_edges(
    ...
    *,
    ...
    edges = gpd.GeoDataFrame(
        {
            ...
            "carrier": _column(lines, "carrier", default="AC", dtype="string").fillna(
                "AC"
            ),
            ...
            "v_nom_kv": (
                model_voltage
                if publish_voltage
                else pd.Series(pd.NA, index=lines.index, dtype="Float64")
            ),
            "s_nom_mva": (
                model_capacity
                if publish_capacity
            ...
            "derived": _column(lines, "derived", default=False, dtype="boolean").fillna(
                False
            ),
            "rating_basis": _column(lines, "rating_basis", dtype="string"),
            "source_route_id": _column(lines, "source_route_id", dtype="string"),
            "source_route_part_id": _column(
                lines, "source_route_part_id", dtype="string"
            ),
    ...
    return edges.loc[:, EDGE_COLUMNS].sort_values(
        "asset_id", kind="mergesort"
    ).reset_index(drop=True)
    ...
    if source_metadata_path is not None:
        ...
        if not source_metadata_path.is_file():
            raise FileNotFoundError(
                f"Network metadata does not exist: {source_metadata_path}"
            )
