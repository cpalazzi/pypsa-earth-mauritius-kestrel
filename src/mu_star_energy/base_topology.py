...
def _meaningful_cycle_count(graph: nx.MultiGraph) -> int:
    ...
    count = sum(
        Polygon(cycle).area >= MEANINGFUL_CYCLE_AREA_M2
    ...
    for node0, node1 in {
        ...
        for attrs in edges[1:]:
            ...
            count += any(
                polygon.area >= MEANINGFUL_CYCLE_AREA_M2 for polygon in polygons
            )
    return count
...
def _split_graph_at_substations(
    ...
    *,
    ...
    connector_points = [
        Point(coordinate)
        for connector in gap_connectors
        for coordinate in (connector.coords[0], connector.coords[-1])
    ]
    graph = nx.MultiGraph()
    for segment in _segments(linework):
        cut_distances = []
...
def _prune_unserved_route_ends(graph: nx.MultiGraph) -> None:
    ...
    while True:
        leaves = [
            node
            for node in graph
            if len(graph[node]) <= 1 and not graph.nodes[node].get("bus_id")
        ]
        if not leaves:
            return
...
def derive_base_topology(
    ...
    *,
    ...
    gap_connectors = _deduplicate_connectors(
        [*station_connectors, *local_connectors, *provided_connectors]
    )
    ...
    cycle_rank = graph.number_of_edges() - graph.number_of_nodes() + nx.number_connected_components(
        graph
    )
    ...
    retained_source_length_km = float(
        sum(
            attrs["length_km"]
            for *_, attrs in sorted_edges
            if attrs["source"] == "provided_transmission_geometry"
        )
    )
    endpoint_pairs = {
        frozenset((node0, node1))
        for node0, node1, _, _ in sorted_edges
        if node0 != node1
    }
    parallel_edge_count = sum(
        max(0, graph.number_of_edges(*tuple(pair)) - 1)
