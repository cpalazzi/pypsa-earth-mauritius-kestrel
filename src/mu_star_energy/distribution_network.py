...
def build_inferred_distribution_graph(
    ...
    *,
    ...
    for row in geographic_assets.itertuples():
        ...
        asset_kind = str(
            getattr(row, "asset_kind", getattr(row, "kind", "substation"))
        ).lower()
    ...
    candidates_by_source = {
        source: [
            node
            for node in candidates
            if source in graph.nodes[node].get("line_sources", set())
        ]
        for source in {
            source
            for node in candidates
            for source in graph.nodes[node].get("line_sources", set())
        }
    }
    for row in geographic_assets.itertuples():
        asset_id = str(getattr(row, "asset_id", getattr(row, "bus_id", "")))
        ...
        graph.nodes[asset_node]["anchor_distance_m"] = min(
            distance_m for _, _, distance_m in anchors
        )
...
def topology_disconnection_impacts(
    ...
    *,
    ...
    root_nodes = {
        ...
        if attrs.get("is_root", attrs.get("kind") == "substation")
    }
