...
def validate_model_tables(
    ...
    *,
    ...
    if generators.empty and not generator_missing:
        warnings.append(
            "No generators are present; this topology-only network cannot supply demand."
        )
    ...
    if "marginal_cost_basis" in generators:
        proxy_costs = (
            generators["marginal_cost_basis"]
            .astype(str)
            .str.contains("proxy", case=False, na=False)
        )
        if proxy_costs.any():
            warnings.append(
    ...
    total_generator_output_capacity_mw = (
        round(float(generator_capacities.loc[complete_generators].sum()), 9)
        if not generator_capacities.empty
        else 0.0
    )
    ...
    if reference_generation_capacity_mw is None:
        ...
        relative_difference = abs(
            total_generator_output_capacity_mw - reference_generation_capacity_mw
        ) / float(reference_generation_capacity_mw)
        coverage_fraction = total_generator_output_capacity_mw / float(
