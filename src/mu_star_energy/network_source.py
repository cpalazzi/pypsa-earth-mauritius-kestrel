...
def _provided_power_assets(
    ...
    if missing:
        formatted = "\n".join(f"- {path}" for path in missing)
        raise FileNotFoundError(
            "Cannot build the provided-data inferred network until these "
...
def _build_inferred_network(
    *,
    ...
    generators = (
        _provided_generators_for_inferred(provided_generators)
        if source == "inferred-provided"
...
def build_network(
    ...
    *,
    ...
    if source not in valid_sources:
        raise ValueError(
            "source must be 'base', 'inferred-osm', or 'inferred-provided'"
        )
